...
// Prototype a pass to check that clock signals are routed properly.
use crate::{
    path::{bit_range, Path},
    schematic::{
        components::{
        ...
        schematic_impl::{PinIx, Schematic},
    },
};
use anyhow::bail;
use anyhow::Result;
...
fn make_index(schematic: &Schematic) -> IndexType {
    let mut index = IndexType::default();
    for wire in &schematic.wires {
        index.entry(wire.source).or_default().insert(wire.dest);
    }
    index
}
...
fn collect_sinks(is: &IndexedSchematic, source: PinPath) -> Result<Vec<PinPath>> {
    // First, find all of the downstream pins
    let downstreams = get_downstream_pin_paths(is, source)?;
    let mut descendents = vec![];
    for pin_path in &downstreams {
        if let Some(children) = is.index.get(&pin_path.pin) {
            for child in children {
                let child_pin_path = PinPath {
                    pin: *child,
                    path: pin_path.path.clone(),
                };
                let child_sinks = collect_sinks(is, child_pin_path)?;
                descendents.extend(child_sinks);
            }
        }
    }
    let mut ret = downstreams.clone();
    ret.extend(descendents);
    Ok(ret)
}

struct IndexedSchematic {
    schematic: Schematic,
    index: IndexType,
}

fn make_indexed_schematic(schematic: Schematic) -> IndexedSchematic {
...
pub fn follow_pin(schematic: Schematic, pin_path: PinPath) -> Result<Vec<PinPath>> {
    let is = make_indexed_schematic(schematic);
    collect_sinks(&is, pin_path)
}
