...
pub struct Sig<T: Digital, C: Clock> {
    val: T,
    clock: std::marker::PhantomData<C>,
}

pub fn signal<T: Digital, C: Clock>(val: T) -> Sig<T, C> {
    Sig {
        val,
...
impl<T: Digital, C: Clock> Sig<T, C> {
    pub fn val(&self) -> T {
        self.val
    }
}
...
impl<T: Digital, C: Clock, const M: usize, const N: usize> std::ops::Index<Sig<Bits<N>, C>>
    for Sig<[T; M], C>
{
    type Output = T;
...
impl<T: Digital, C: Clock, const M: usize, const N: usize> std::ops::IndexMut<Sig<Bits<N>, C>>
    for Sig<[T; M], C>
{
...
macro_rules! impl_assign_op {
    ($trait: ident, $op: ident) => {
        impl<T: Digital + std::ops::$trait, C: Clock> std::ops::$trait for Sig<T, C> {
...
macro_rules! impl_shift_assign_op {
    ($trait: ident, $op: ident) => {
        impl<T: Digital + std::ops::$trait, C: Clock> std::ops::$trait<T> for Sig<T, C> {
...
macro_rules! impl_cmpop {
    ($trait: ident, $op: ident, $ret: ty) => {
        // Case for Sig == Sig
        impl<T: Digital + std::cmp::$trait, C: Clock> std::cmp::$trait<Sig<T, C>> for Sig<T, C> {
        ...
        impl<const N: usize, C: Clock> std::cmp::$trait<u128> for Sig<Bits<N>, C> {
        ...
        impl<const N: usize, C: Clock> std::cmp::$trait<i128> for Sig<SignedBits<N>, C> {
        ...
        impl<const N: usize, C: Clock> std::cmp::$trait<Sig<Bits<N>, C>> for u128 {
        ...
        impl<const N: usize, C: Clock> std::cmp::$trait<Sig<SignedBits<N>, C>> for i128 {
        ...
        impl<T: Digital + std::cmp::$trait, C: Clock> std::cmp::$trait<T> for Sig<T, C> {
        ...
        impl<const N: usize, C: Clock> std::cmp::$trait<Sig<Bits<N>, C>> for Bits<N> {
            fn $op(&self, rhs: &Sig<Bits<N>, C>) -> $ret {
                std::cmp::$trait::$op(self, &rhs.val)
            }
        }
        ...
        // Case for signed == Sig
        impl<const N: usize, C: Clock> std::cmp::$trait<Sig<SignedBits<N>, C>> for SignedBits<N> {
            fn $op(&self, rhs: &Sig<SignedBits<N>, C>) -> $ret {
                std::cmp::$trait::$op(self, &rhs.val)
            }
        }
...
macro_rules! impl_shiftop {
    ($trait: ident, $op: ident) => {
        // Case for Sig << Sig
        impl<T: Digital + std::ops::$trait<Output = T>, C: Clock> std::ops::$trait<Sig<T, C>>
            for Sig<T, C>
        {
            type Output = Sig<T, C>;
        ...
        impl<const N: usize, C: Clock> std::ops::$trait<u128> for Sig<Bits<N>, C> {
        ...
        // Case for constant << Sig
        impl<const N: usize, C: Clock> std::ops::$trait<Sig<Bits<N>, C>> for Bits<N> {
            type Output = Sig<Bits<N>, C>;
        ...
        // Case for signed << Sig
        impl<const N: usize, C: Clock> std::ops::$trait<Sig<Bits<N>, C>> for SignedBits<N> {
...
macro_rules! impl_binop {
    ($trait: ident, $op: ident) => {
        ...
        // Case for Sig + literal (signed)
        impl<const N: usize, C: Clock> std::ops::$trait<i128> for Sig<SignedBits<N>, C> {
            type Output = Sig<SignedBits<N>, C>;
        ...
        // Case for literal + Sig (unsigned)
        impl<const N: usize, C: Clock> std::ops::$trait<Sig<Bits<N>, C>> for u128 {
            type Output = Sig<Bits<N>, C>;
        ...
        // Case for literal + Sig (signed)
        impl<const N: usize, C: Clock> std::ops::$trait<Sig<SignedBits<N>, C>> for i128 {
            type Output = Sig<SignedBits<N>, C>;
        ...
        impl<const N: usize, C: Clock> std::ops::$trait<Sig<SignedBits<N>, C>> for SignedBits<N> {
            type Output = Sig<SignedBits<N>, C>;
...
impl<T: Digital + std::ops::Not<Output = T>, C: Clock> std::ops::Not for Sig<T, C> {
    type Output = Sig<T, C>;
...
impl<T: Digital + std::ops::Neg<Output = T>, C: Clock> std::ops::Neg for Sig<T, C> {
    type Output = Sig<T, C>;
...
impl<T: Digital, const M: usize, const N: usize, C: Clock> std::ops::Index<Sig<Bits<N>, C>>
    for [T; M]
where
    [T; M]: Digital,
{
