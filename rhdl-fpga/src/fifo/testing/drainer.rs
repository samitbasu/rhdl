...
pub struct U<const N: usize> {
    ...
    read_probability: constant::U<Bits<32>>,
    valid: dff::U<bool>,
}
...
impl<const N: usize> Default for U<N> {
    fn default() -> Self {
        Self {
            ...
            read_probability: constant::U::new(bits(0xD000_0000)),
...
impl<const N: usize> U<N> {
    pub fn new(sleep_len: u8, read_probability: u32) -> Self {
        Self {
            rng: crate::rng::xorshift::U::default(),
            sleep_counter: dff::U::new(bits(0)),
...
impl<const N: usize> SynchronousIO for U<N> {
    type I = Option<Bits<N>>;
    type O = O;
    type Kernel = drain_kernel<N>;
}

#[kernel]
pub fn drain_kernel<const N: usize>(cr: ClockReset, input: Option<Bits<N>>, q: Q<N>) -> (O, D<N>) {
    let mut d = D::<N>::init();
    let mut o = O::init();
    ...
    let (data_available, data) = match input {
        Some(data) => (true, data),
        None => (false, bits(0)),
    };
    ...
    if will_read {
        ...
        d.sleep_counter = if q.rng < q.read_probability {
            q.sleep_len
        } else {
            bits(0)
        }
