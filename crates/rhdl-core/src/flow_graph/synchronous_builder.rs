...
pub fn build_synchronous_flowgraph<C: Synchronous>(
    ...
    for path in input_kind.all_leafs() {
        ...
        let kernel_input = kernel_map.input_port(1, &path)?;
        builder.add_edge(
            input_port,
            kernel_input,
    ...
    for child_descriptor in children {
        ...
        for path in child_input_kind.all_leafs() {
            ...
            let child_input_port = child_remap.input_port(1, &path)?;
        ...
        for path in child_output_kind.all_leafs() {
            ...
            let kernel_input_port =
                kernel_map.input_port(2, &Path::default().field(child_name).join(&path))?;
            builder.add_edge(
                child_output_port,
                kernel_input_port,
