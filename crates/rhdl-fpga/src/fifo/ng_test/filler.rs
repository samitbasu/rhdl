...
pub struct In {
    /// Input from the `ready` signal of the FIFO
    pub ready: bool,
}
...
mod tests {
    ...
    fn test_filler() -> miette::Result<()> {
        ...
        let input = std::iter::repeat_n(In { ready: true }, 50)
            .with_reset(1)
            .clock_pos_edge(100);
        ...
        let expect = expect!["2d3d5f1dd0ca55061507adf19fcd44b9b00838b15034df92b5c408657b4bf59a"];
        let digest = vcd.dump_to_file(root.join("filler_ng.vcd")).unwrap();
    ...
    fn test_filler_testbench() -> miette::Result<()> {
        ...
        let expect = expect_file!["filler_ng.expect"];
