...
```rust
{{#rustdoc_include ../code/src/half_adder.rs:adder-step-3}}
```
...
|trait|derive reason|
|-----|--------------|
|`Digital`|Needed for all synthesizable data types|
|`Copy`| `Digital` requires `Copy`|
|`Clone`| `Copy` requires `Clone` |
|`Timed` | Adds `Timed` trait if all members are also `Timed` |
|`PartialEq`| Needed for comparison operators to work |
...
```rust
{{#rustdoc_include ../code/src/half_adder.rs:adder-step-4}}
```
...
```rust
{{#rustdoc_include ../code/src/half_adder.rs:adder-step-5}}
```
...
```rust
{{#rustdoc_include ../code/src/half_adder.rs:adder-step-6}}
```

and the `Q` type must be defined as

```rust
{{#rustdoc_include ../code/src/half_adder.rs:adder-step-7}}
```

These types must follow the pattern of having

- Each field name in `D` and `Q` must map _exactly_ to a field in the parent circuit (the `HalfAdder`)
...
```rust
{{#rustdoc_include ../code/src/half_adder.rs:adder-step-8}}
```

The last step is to write the kernel function.  In this case, the signature of the kernel is

```rust
{{#rustdoc_include ../code/src/half_adder.rs:adder-step-9}}
```

The kernel function must:

- given the current inputs `i: Signal<(bool,bool), Red>`
- given the current outputs of internal components `q: HalfAdderQ`

compute the circuit output `O` and the inputs to the internal components `d: HalfAdderD`.  In this case, the kernel function is fairly straightforward.  The subcircuits are simply to be fed copies of the input signals, and the output is collected from their computed outputs:

```rust
{{#rustdoc_include ../code/src/half_adder.rs:adder-step-10}}
```

The output is computed by taking the sum as the output of the `xor` gate, which is `q.xor`, and the carry as the output of the `and` gate, which is `q.and`.  The complete circuit definition is thus:

```rust
{{#rustdoc_include ../code/src/half_adder.rs:adder-step-11}}
```
