...
```rust
{{#rustdoc_include digital_ex/src/derive_ex.rs:struct}}
```
...
```rust
{{#rustdoc_include digital_ex/src/derive_ex.rs:main}}
```
and confirm that the resulting SVG is the same as before:

![Things SVG (autoderived)](digital_ex/things_derived.svg)
...
```rust
{{#rustdoc_include digital_ex/src/derive_enum.rs:enum}}
```

We see that this does _not_ compile.  

<!-- cmdrun to-html "cd digital_ex && cargo check --features no-default-impl 2>&1" -->
...
```rust
{{#rustdoc_include digital_ex/src/derive_enum.rs:enum-fixed}}
```

With the following test definition:

```rust
{{#rustdoc_include digital_ex/src/derive_enum.rs:main}}
```

we get the following SVG layout:

![Things SVG (autoderived)](digital_ex/opcode_derived.svg)
