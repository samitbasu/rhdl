...
```rust
{{#rustdoc_include digital_ex/src/struct_ex.rs:prelude}}
```

The following struct, for example:

```rust
{{#rustdoc_include digital_ex/src/struct_ex.rs:struct-definition}}
```
...
```rust
{{#rustdoc_include digital_ex/src/struct_ex.rs:struct-BITS}}
```
...
```rust
{{#rustdoc_include digital_ex/src/struct_ex.rs:struct-def-with-derive}}
```
...
```rust
{{#rustdoc_include digital_ex/src/struct_ex.rs:struct-static_kind}}
```
...
```rust
{{#rustdoc_include digital_ex/src/struct_ex.rs:struct-bin}}
```

Similarly, the `dont_care` method can be delegated as well:

```rust
{{#rustdoc_include digital_ex/src/struct_ex.rs:struct-dont_care}}
```
...
```rust
{{#rustdoc_include digital_ex/src/struct_ex.rs:struct-layout}}
```

If you run the test, it will generate the SVG output

![Things SVG](digital_ex/things.svg)
