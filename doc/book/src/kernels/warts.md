...
where
   ...
   rhdl_bits::W<{N+1}>: BitWidth
{}
```

But this requires unstable nightly features, and I simply don't want to go there.  For now, the workaround is to either use a macro to write the function (gross), or pass the responsibility to the caller (also gross), and then add an assertion to protect against incorrect use.  For the later approach, you might do:

```rust
fn extend<const N: usize, const NP1: usize>(a: Bits::<N>) -> Bits::<NP1>
where
    rhdl_bits::W<N>: BitWidth,
    rhdl_bits::W<NP1>: Bitwidth
{
    assert_eq!(N + 1, NP1);
}
```
